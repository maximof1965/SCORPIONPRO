...
async function upsertCajaDelta(fechaISO, turno, campo, valor) {
  ...
  // Incremento ATOMICO en la base de datos (INSERT ... ON CONFLICT DO UPDATE
  // con "campo = campo + delta" en una sola sentencia). Antes se hacia
  // leer -> sumar en JS -> escribir, lo que ante operaciones concurrentes
  // provocaba "lost updates" (contratos que no se sumaban y descuadraban CAJA).
  const { error } = await supabase.rpc('incrementar_caja', {
    p_fecha: fechaCaja,
    p_turno: turno,
    p_campo: campo,
    p_valor: toNumber(valor),
  });
