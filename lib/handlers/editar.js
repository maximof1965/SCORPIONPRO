...
// LISTADO
// --------------------------------------------------------------
async function listarOperaciones({ turno, tipo, fechaStr }) {
  const fechaBase = (fechaStr && fechaStr.length >= 10)
    ? fechaStr.slice(0, 10)
    : new Date().toISOString().slice(0, 10);

  const desde = `${fechaBase}T00:00:00Z`;
  const hasta = `${fechaBase}T23:59:59Z`;
...
module.exports = async (req, res) => {
  try {
    ...
    if (accion === 'list') {
      ...
      const data = await listarOperaciones({ turno, tipo, fechaStr });
