...
async function editarAumento(original, nuevo) {
  ...
  // El AUMENTO original tiene dos numeros:
  //   VALOR     = monto del aumento (afecta VR PR + CAJA.CONTRATOS)
  //   DETALLE   = "AUMENTO:X - PAGO_AUMENTO:Y" (Y afecta VR RT)
  ...
  // CAJA.CONTRATOS solo refleja el AUMENTO (el dinero efectivamente
  // entregado al cliente). El PAGO_AUMENTO se cobrara despues.
  await ajustarCaja(original.FECHA, original.OPER, 'CONTRATOS', dAum);
...
async function editarDevolucion(original, nuevo) {
  ...
  const updates = {
    ...
    'TOT IN': Math.max(0, totInReset - restaInNew)
  };
  const futuro = { ...empe, ...updates };
  updates.TOTAL = calcularTotal(futuro);
  ...
  // 3) CAJA: la devolucion es egreso -> resta de RETIRO. Aplicar el delta.
  await ajustarCaja(original.FECHA, original.OPER, 'RETIRO', -delta(devAntes, devNuevo));
