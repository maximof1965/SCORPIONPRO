...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ============================================================
    // 3.b) CAJA HISTORICA COMPLETA (SIN filtro de fechas)
    //      Para la tabla mensual del administrador: se agrega
    //      por YYYY-MM y se devuelven los totales del mes.
    //      A proposito NO respeta el filtro Desde/Hasta porque
    //      el administrador quiere ver la historia completa.
    // ============================================================
    const cajaHistorica = await traerTodo(
      'CAJA',
      'FECHA, BASE, RETIRO, GUARDA, BAR, CONTRATOS, GASTOS, VENTAS, "CUADRE CAJA"',
      null, null, null
    );
    ...
    // i) CAJA agregada por mes (toda la historia)
    //    Devolvemos siempre las columnas que pidio el negocio:
    //    BASE, RETIRO, GUARDA, BAR, CONTRATOS, GASTOS, VENTAS, CUADRE_CAJA
    //    El frontend dibuja una tabla con MES/AÑO + estas columnas.
    const cajaMensualMap = {};
    for (const c of cajaHistorica) {
      ...
      const key = `${y}-${m}`;
      if (!cajaMensualMap[key]) {
        cajaMensualMap[key] = {
          mes: key,
          anio: y,
          mes_numero: d.getUTCMonth() + 1,
          BASE: 0,
          RETIRO: 0,
          GUARDA: 0,
          BAR: 0,
      ...
      const acc = cajaMensualMap[key];
      acc.BASE        += Number(c.BASE || 0);
      acc.RETIRO      += Number(c.RETIRO || 0);
      acc.GUARDA      += Number(c.GUARDA || 0);
      acc.BAR         += Number(c.BAR || 0);
      acc.CONTRATOS   += Number(c.CONTRATOS || 0);
      acc.GASTOS      += Number(c.GASTOS || 0);
      acc.VENTAS      += Number(c.VENTAS || 0);
      acc['CUADRE CAJA'] += Number(c['CUADRE CAJA'] || 0);
      acc.dias++;
    }
    const cajaMensual = Object.values(cajaMensualMap).sort((a, b) =>
