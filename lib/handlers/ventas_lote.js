...
function turnoElegidoPorOperador(valor) {
  if (valor == null) return null;
  const t = String(valor).trim();
  return /^[123]$/.test(t) ? t : null;
}
...
module.exports = async (req, res) => {
  try {
    ...
    // El operador elige el turno desde el frontend.
    // Si por algún motivo no llegara, hacemos fallback a la hora local
    // (compat con clientes antiguos), pero el frontend nuevo SIEMPRE lo manda.
    const turnoOperador = turnoElegidoPorOperador(body.turno);
    ...
    const turno = turnoOperador || detectarTurnoActual();
